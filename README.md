...
* Starting with GATK4.2.3 genotypes without sequencing coverage are now called as homozygous reference (e.g. 0/0) instead of no-call (e.g. ./.) as before. According to GATK [this is not a bug but a feature](https://gatk.broadinstitute.org/hc/en-us/articles/6012243429531-GenotypeGVCFs-and-the-death-of-the-dot), unfortunately it took the GATK team more than two years to tell anybody about it. In this workflow this issue gets fixed for biallelic and invariant sites by setting depth thresholds for genotypes under which sites are set to no-call.
