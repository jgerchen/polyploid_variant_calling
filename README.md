...
* **config[report_dir]/bam_depth/{species}\_{sample}.chr.stat.gz** gzipped table with raw sequencing depth calculated by PanDepth (including duplicates and low quality reads) per contig for a single individual
* **config[report_dir]/bam_depth/{species}\_{sample}.chr.clean.stat.gz** gzipped table with filtered sequencing depth calculated by PanDepth (duplicates and reads with mapping quality < **pandepth_min_q** removed) per chromosome for a single individual
* **config[report_dir]/bam_depth/{species}\_{sample}.win.stat.gz** gzipped table with filtered sequencing depth calculated by PanDepth (duplicates and reads with mapping quality < **pandepth_min_q** removed) in sliding windows with size**pandepth_window_size** for a single individual
* **config[report_dir]/merge_bam_stats/{species}.bam.table.tsv** tab separated table collecting relevant results of flagstats and bamDepth for all samples
...
* **config[vcf_filtered]/{species}.bigt.dp.m.bt.vcf.gz** Biallelic SNPs, with >**gen_max_missing** missing data removed
...
* for GATK versions 4.2.3 to 4.5.0.0 genotypes without sequencing coverage are called as homozygous reference (e.g. 0/0) instead of no-call (e.g. ./.) as before. Originally GATK [considered this to be not a bug but a feature](https://gatk.broadinstitute.org/hc/en-us/articles/6012243429531-GenotypeGVCFs-and-the-death-of-the-dot), unfortunately it took the GATK team more than two years to tell anybody about it. However, after people realized and after faulty genotype calls may have been implemented in numerous (potentially clinically relevant) studies, GATK reversed this to the old behaviour in GATK 4.6.0.0. In this workflow this issue gets fixed for biallelic and invariant sites by setting depth thresholds for genotypes under which sites are set to no-call.
